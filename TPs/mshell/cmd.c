...
{
  struct job_t *jobp;
  jobp = treat_argv(argv);
  if (jobp==NULL) perror("treat_argv()failed");
  kill(jobp->jb_pid,SIGCONT);
  jobp->jb_state=BG;
  return;
}
...
{ 
  pid_t currentFGjob;
  currentFGjob = jobs_fgpid();
  if (currentFGjob ==0)
    printf("Il n'y a aucun job en foreground.\n");
  else {
...
do_kill(char **argv) 
{
  ...
  jobp = treat_argv(argv);
  if (jobp==NULL) perror("treat_argv()failed");
  kill(jobp->jb_pid,SIGKILL);
  jobs_deletejob(jobp->jb_pid);
  return;
}
...
do_exit()
{ 
  /* si il n'y a pas de jobs stoppé */
